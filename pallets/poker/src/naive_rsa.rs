...
// type Result = Result<Vec<u8>, &'static str>;

pub fn encrypt(data: &[u8], pubkey: &[u8]) -> DispatchResult {
    let exponent: U256 = U256::from(65537);
    generic_crypter(data, pubkey, exponent)
}

pub fn decrypt(data: &[u8], pubkey: &[u8], privkey: &[u8]) -> DispatchResult {
...
fn generic_crypter(data: &[u8], pubkey: &[u8], exponent: U256) -> DispatchResult {
    if data.len() > 32 {
